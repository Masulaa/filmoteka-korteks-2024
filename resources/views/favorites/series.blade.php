...
@section('content')
    ...
    <script>
        function removeFromFavorites(serieId) {
            ...
            .then(data => {
                ...
                document.getElementById(`serie-card-${serieId}`).remove();
            })
            .catch(error => {
    ...
    <div class="min-h-screen py-12 transition-colors duration-300 bg-gray-100 dark:bg-gray-900">
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <h1 class="mb-8 text-3xl font-extrabold text-gray-900 dark:text-white animate-fade-in">My Favorite Series</h1>

            @if ($favoriteSeries->count() > 0)
                            ...
                            <a href="{{ route('series.index') }}/ {{ $serie->id }}" class="block">
                                <div class="relative aspect-w-2 aspect-h-3">
                                    ...
                                    <div class="absolute inset-0 flex items-center justify-center transition-all duration-300 bg-black bg-opacity-50 opacity-0 group-hover:scale-110 group-hover:opacity-100">
                                        <button
                                            onclick="event.stopPropagation(); event.preventDefault(); removeFromFavorites({{ $serie->id }});"
                                ...
                                <div class="px-4 py-3 mt-2">
                                    ...
                                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ $serie->genre }}</p>
                                    <p class="mt-1 text-sm font-medium text-gray-700 dark:text-gray-300">Directed by:
                                        {{ $serie->director }}</p>
                                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                                        {{ Str::limit($serie->overview, 100) }}</p>
            ...
            @else
                <div class="text-center animate-fade-in">
                    <p class="text-xl text-gray-600 dark:text-gray-400">You have no favorite series.</p>
                </div>
            @endif
        </div>
    </div>
