...
use App\Models\{Serie, SerieCast};
...
class SeriesService
{
    ...
    {
        return $this->tmdbService->fetchData('https://api.themoviedb.org/3/tv/popular', ['page' => 1])['total_results'] ?? 0;
    }
    ...
    {
        $cast = array_slice($this->tmdbService->fetchData("https://api.themoviedb.org/3/tv/{$seriesId}/credits", [])['cast'] ?? [], 0, 10);
    ...
    public function createOrUpdateSeries(array $seriesData): void
    {
        ...
        $castData = $this->getCast($seriesData['id']);
        $castEntries = [];
     ...
     * @return void
     */
    private function processSeriesData(array $seriesData, int &$syncCount, int $numberOfSeriesToDownload, int &$skipCount, int &$newCount, bool $consoleOutput): void
    {
        if (Serie::where('title', $seriesData['name'])->exists()) {
            $consoleOutput && printf("\033[K($syncCount/$numberOfSeriesToDownload) Series '{$seriesData['name']}' already exists.\033[35m Skip\033[0m.\n");
            $skipCount++;
            return;
        }
        $this->createOrUpdateSeries($seriesData);
        $newCount++;
        $consoleOutput && printf("\033[K($syncCount/$numberOfSeriesToDownload) \033[33mNew\033[0m series '{$seriesData['name']}' added to the database.\n");
    }
    ...
    public function fetchPopularSeries(int $numberOfSeriesToDownload, bool $consoleOutput=false): void
    {
        ...
        $consoleOutput && printf("\033[KSuccessfully synchronized {$syncCount} series.\nSkip: {$skipCount}\nNew: {$newCount}\n");
    }
}
