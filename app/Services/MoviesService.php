...
class MoviesService
{
     ...
     * @return void
     */
    private function processMovieData(
        array $movieData,
        int &$syncCount,
        int $numberOfMoviesToDownload,
        int &$skipCount,
        int &$newCount,
        bool $consoleOutput
    ): void {
        if (Movie::where("title", $movieData["title"])->exists()) {
            $consoleOutput &&
                printf(
                    "\033[K($syncCount/$numberOfMoviesToDownload) Movie '{$movieData["title"]}' already exists.\033[35m Skip\033[0m.\n"
                );
            $skipCount++;
            return;
        }
        ...
        $consoleOutput &&
            printf(
                "\033[K($syncCount/$numberOfMoviesToDownload) \033[33mNew\033[0m movie '{$movieData["title"]}' added to the database.\n"
            );
    }
    ...
    public function fetchPopularMovies(
        ...
        $consoleOutput &&
            printf("\033[34m::\033[0m Synchronizing movies from TMDb...\n");
        ...
        while ($page++ < $totalPages) {
            ...
            foreach ($moviesData as $movieData) {
                ...
                $syncCount++;
                $progress = floor(
                    ($syncCount / $numberOfMoviesToDownload) * 100
                );
        ...
        $consoleOutput &&
            printf(
                "\033[KSuccessfully synchronized {$syncCount} movies.\nSkip: {$skipCount}\nNew: {$newCount}\n"
            );
    }
}
